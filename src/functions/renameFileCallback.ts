...
export function renameFile(oldFilePath: string, newFileName: string, callback: (error: Error | null) => void) {
    const oldFileName = path.basename(oldFilePath);
    const uniqueSuffix = Date.now() + "-" + Math.round(Math.random() * 1e9);
    const randomPrefix = Math.random().toString(36).substring(2, 8); // Genera un prefijo aleatorio de 6 caracteres
    const finalFileName = randomPrefix + '-' + newFileName + '-' + uniqueSuffix;
